...
void ClassicAppMode::StartKickVote(scene_v1::ConnectionToClient* starter,
  ...
  if (starter == target) {
    ...
    // Two chat messages sent as the server:
    //   1) "a kick vote started by X against Y"
    //   2) "Type 1 if you want to kick him/her or 0 if not"
    std::string kick_starter_name =
        starter->GetCombinedSpec().GetDisplayString();
    std::string kick_target_name =
        target->GetCombinedSpec().GetDisplayString();
    const std::string kick_msg1 =
        "a kick vote started by " + kick_starter_name
        + " against " + kick_target_name;
    const std::string kick_msg2 =
        "Type 1 if you want to kick him/her or 0 if not";

    // Build BA_MESSAGE_CHAT packets — same format as ConnectionToClient uses
    // when forwarding real chat, so LocalDisplayChatMessage on every side
    // stores them in chat_messages_ and shows them in the party window.
    std::string host_spec =
        scene_v1::PlayerSpec::GetAccountPlayerSpec().GetSpecString();
    size_t spec_size = host_spec.size();
    assert(spec_size < 256);

    auto MakeChatPacket = [&](const std::string& msg) {
      std::vector<uint8_t> buf(1 + 1 + spec_size + msg.size());
      buf[0] = BA_MESSAGE_CHAT;
      buf[1] = static_cast<uint8_t>(spec_size);
      memcpy(&(buf[2]), host_spec.c_str(), spec_size);
      memcpy(&(buf[2 + spec_size]), msg.c_str(), msg.size());
      return buf;
    };

    auto pkt1 = MakeChatPacket(kick_msg1);
    auto pkt2 = MakeChatPacket(kick_msg2);

    // Deliver both to every connected client → stored in their party window.
    for (auto&& client : connected_clients) {
      if (client->can_communicate()) {
        client->SendReliableMessage(pkt1);
        client->SendReliableMessage(pkt2);
      }
    }

    // Display both on the HOST side → stored in host's party window.
    LocalDisplayChatMessage(pkt1);
    LocalDisplayChatMessage(pkt2);
